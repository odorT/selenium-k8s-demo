...
if __name__ == '__main__':
    ...
    if os.getenv("K8S_PROVIDER") == "EKS":
        cluster_name = os.getenv("K8S_CLUSTER_NAME", "selenium-demo")
        v1, apps = load_aws_config(cluster_name)
    else:
        v1, apps = load_minikube_config()

    test_runner_pod_count = 3

    ns_name = create_ns(v1, apps)
    svc_name = create_svc(v1, apps, ns_name)
    deploy_name = create_deploy(v1, apps, ns_name, test_runner_pod_count)
